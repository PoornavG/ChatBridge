...
class ClientApp:
    ...
    def authenticate(self):
        has_account = simpledialog.askstring("Login", "Do you have an account? (yes/no):").strip().lower()
        self.client_socket.send(has_account.encode())

        if has_account == "no":
            new_username = simpledialog.askstring("Register", "Enter a new username:")
            self.client_socket.send(new_username.encode())
